...
async function extractTextFromURL(url) {
  try {
    console.log(`🔗 Extracting content from: ${url} using ScraperAPI with JavaScript rendering`);
    ...
    const response = await axios.get(scraperApiUrl, {
      ...
      timeout: 60000, // 60 seconds timeout for JavaScript rendering
    });
...
Try uploading a different URL or use a supported website.`,
    ...
    console.log(`✅ ScraperAPI with JS rendering: Extracted ${text.length} characters from ${url}`);
