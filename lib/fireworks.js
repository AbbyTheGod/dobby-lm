...
STYLE RULES (Plain text; no markdown):
- Use blank lines between sections
- Keep sentences concise and readable
- Use bullet points with clear spacing
- Use numbered lists when appropriate
- Do not use markdown syntax (no **bold**, *italic*, headings)
- Do not use brackets for emphasis; brackets are reserved for citations only
`.trim();
...
function buildSystemPrompt({ roleName = 'Dobby', sources = 'No sources available', citationFormat = '[S{source_id}:{chunk_index}]', extra = '' } = {}) {
  return `
You are ${roleName}, a helpful AI assistant. You write in plain text only.

${PLAIN_TEXT_STYLE_RULES}

CITATIONS:
- When referencing sources, include citations in this format: ${citationFormat}
- Only cite when a statement comes from the provided sources
...
Key Points:
• First important point with clear spacing
• Second important point with clear structure
• Third important point easy to read

Additional Details:
Provide short paragraphs with blank lines between them.

Main Benefits:
1. First benefit with a brief explanation
2. Second benefit with supporting detail
3. Third benefit, concise
`.trim(),
  });
