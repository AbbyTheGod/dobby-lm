...
const MODEL_NAME = 'accounts/fireworks/models/sentientfoundation-serverless/dobby-mini-unhinged-plus-llama-3-1-8b';

async function callFireworksAPI(messages, options = {}) {
  // Use mock API if Fireworks API key is not available
  if (!process.env.FIREWORKS_API_KEY || process.env.FIREWORKS_API_KEY.includes('your_fireworks_api_key_here')) {
    console.log('🔧 Using mock Fireworks API for testing');
    const { createChatCompletion } = await import('./mock-fireworks.js');
