...
function enforceSpacing(text) {
  return text
    // Fix section headers first - add blank lines after colons
    .replace(/(Key Points|Additional Details|Main Benefits):/g, '$1:\n\n')
    // Fix bullet points - add blank lines before each bullet
    .replace(/• /g, '\n\n• ')
    // Fix numbered lists - add blank lines before each number
    .replace(/(\d+\.\s)/g, '\n\n$1')
    ...
    .replace(/^(Key Points|Additional Details|Main Benefits):/, '\n$1:')
...
REMEMBER: You MUST follow the formatting rules exactly. Use blank lines between every section and bullet point. Never collapse text into one block.`.trim();
}
...
function createChatPrompt(query, context, citations = '[S{source_id}:{chunk_index}]') {
  const systemPrompt = buildSystemPrompt({
    ...
    extra: `
CRITICAL: You MUST include blank lines between every section and every bullet point.
...
MANDATORY RULES:
- ALWAYS put a blank line after "Key Points:"
- ALWAYS put a blank line after each bullet point
- ALWAYS put a blank line after "Additional Details:"
- ALWAYS put a blank line after "Main Benefits:"
- ALWAYS put a blank line after each numbered item
- NEVER write everything in one block without spacing
