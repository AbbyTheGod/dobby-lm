...
=======
  ...
  if (normalizedText.includes(normalizeSQL('SELECT n.*, COUNT(s.id) as source_count, COUNT(m.id) as message_count FROM notebooks n LEFT JOIN sources s ON n.id = s.notebook_id LEFT JOIN messages m ON n.id = m.notebook_id GROUP BY n.id ORDER BY n.updated_at DESC LIMIT $1 OFFSET $2'))) {
  ...
  if (normalizedText.includes(normalizeSQL('SELECT n.*, COUNT(s.id) as source_count, COUNT(m.id) as message_count FROM notebooks n'))) {
    const notebooks = sessionData.notebooks.map(notebook => ({
      ...notebook,
      source_count: sessionData.sources.filter(s => s.notebook_id === notebook.id).length,
      message_count: sessionData.messages.filter(m => m.notebook_id === notebook.id).length
  ...
  if (normalizedText.includes(normalizeSQL('SELECT * FROM notebooks'))) {
    return { rows: sessionData.notebooks };
  }
  
  if (normalizedText.includes(normalizeSQL('SELECT * FROM sources WHERE notebook_id = $1'))) {
    const notebookId = params[0];
    const sources = sessionData.sources.filter(s => s.notebook_id === notebookId);
    return { rows: sources };
  }
  
  if (normalizedText.includes(normalizeSQL('SELECT * FROM sources WHERE id = $1'))) {
    const sourceId = params[0];
    const sources = sessionData.sources.filter(s => s.id === sourceId);
    return { rows: sources };
  }
  
  if (normalizedText.includes(normalizeSQL('SELECT * FROM chunks WHERE source_id = $1'))) {
    const sourceId = params[0];
    const chunks = sessionData.chunks.filter(c => c.source_id === sourceId);
    return { rows: chunks };
  }
  
  if (normalizedText.includes(normalizeSQL('SELECT * FROM chunks'))) {
    return { rows: sessionData.chunks };
  }
  
  if (normalizedText.includes(normalizeSQL('SELECT c.*, s.notebook_id FROM chunks c JOIN sources s ON c.source_id = s.id WHERE s.notebook_id = $1'))) {
    const notebookId = params[0];
  ...
  if (normalizedText.includes(normalizeSQL('SELECT * FROM messages WHERE notebook_id = $1'))) {
    const notebookId = params[0];
    const messages = sessionData.messages.filter(m => m.notebook_id === notebookId);
    return { rows: messages };
  }
  
  if (normalizedText.includes(normalizeSQL('SELECT * FROM study_tools WHERE notebook_id = $1 AND type = $2'))) {
    const notebookId = params[0];
    const type = params[1];
  ...
  if (normalizedText.includes(normalizeSQL('SELECT * FROM study_tools WHERE notebook_id = $1'))) {
    const notebookId = params[0];
  ...
  if (normalizedText.includes(normalizeSQL('SELECT c.*, s.title as source_title FROM chunks c JOIN sources s ON c.source_id = s.id WHERE s.notebook_id = $1 ORDER BY s.created_at, c.chunk_index'))) {
    const notebookId = params[0];
  ...
  if (normalizedText.includes(normalizeSQL('SELECT c.*, s.title as source_title, s.type as source_type FROM chunks c JOIN sources s ON c.source_id = s.id WHERE s.notebook_id = $1 ORDER BY c.embedding <=> $2 LIMIT $3'))) {
    const notebookId = params[0];
  ...
  if (normalizedText.includes(normalizeSQL('INSERT INTO notebooks'))) {
    const newNotebook = {
      id: getNextId(),
      title: params[0],
  ...
  if (normalizedText.includes(normalizeSQL('INSERT INTO sources'))) {
    const newSource = {
      id: getNextId(),
  ...
  if (normalizedText.includes(normalizeSQL('INSERT INTO chunks'))) {
    const newChunk = {
      id: getNextId(),
      source_id: params[0],
      content: params[1],
  ...
  if (normalizedText.includes(normalizeSQL('INSERT INTO messages'))) {
    const newMessage = {
      id: getNextId(),
      notebook_id: params[0],
      role: params[1],
  ...
  if (normalizedText.includes(normalizeSQL('INSERT INTO study_tools'))) {
    const newStudyTool = {
      id: getNextId(),
      notebook_id: params[0],
      type: params[1],
  ...
  if (normalizedText.includes(normalizeSQL('UPDATE notebooks SET updated_at = CURRENT_TIMESTAMP WHERE id = $1'))) {
    const notebookId = params[0];
  ...
  if (normalizedText.includes(normalizeSQL('UPDATE sources SET status = $1 WHERE id = $2'))) {
  ...
  if (normalizedText.includes(normalizeSQL('UPDATE sources SET status = $1, updated_at = NOW() WHERE id = $2'))) {
    const status = params[0];
    const sourceId = params[1];
    const source = sessionData.sources.find(s => s.id === sourceId);
    if (source) {
  ...
  return { rows: [] };
>>>>>>> 5c1749138d707c43815bed81970fcb58119bd546
}
